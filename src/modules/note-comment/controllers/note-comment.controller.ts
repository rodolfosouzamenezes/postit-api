...
export class NoteCommentController {
  constructor(private readonly service: NoteCommentService) {}
  ...
  public createOne(
    @User() requestUser: UserEntity,
    @Param('noteId') noteId: string,
    @Body() payload: CreateNoteCommentPayload,
  ): Promise<NoteCommentProxy> {
    return this.service
      .createOne(requestUser, +noteId, payload)
      .then((entity) => new NoteCommentProxy(entity));
  }
  ...
  public deleteOne(
    @User() requestUser: UserEntity,
    @Param('noteId') noteId: string,
    @Param('noteCommentId') noteCommentId: string,
  ): Promise<void> {
